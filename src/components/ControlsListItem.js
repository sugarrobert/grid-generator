...
function ControlsListItem(props) {
    ...
    return (
        <div className="controls__list__item" id={props.id}>
            ...
            <input id={props.input.id} placeholder="1" type="number" min="1" value={props.input.value} aria-label="{`Grid Template ${props.name} amount`}" className="input column__value" onChange={props.onValueChange}/>
            <label htmlFor={props.options.id} className="no-display">{`Grid Template ${props.name} unit`}</label>
            <select id={props.options.id} className="column__value" aria-label="{`Grid Template ${props.name} unit`}">
                {
                    props.options.option.map(option => 
